...
class ModelArgs:
    ...
    n_kv_heads: Optional[float] = None # Number of heads for the K and V
...
def apply_rotart_embeddings(x: torch.Tensor, freqs_complex: torch.Tensor, device):
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_complex: torch.Tensor):
        ...
        # (B, 1, H_Q * Head_Dim) -> (B, 1, H_Q, Head_Dim)
        xq = apply_rotart_embeddings(xq, freqs_complex, device = x.device)
        # (B, 1, H_KV * Head_Dim) -> (B, 1, H_KV, Head_Dim)
        xk = apply_rotart_embeddings(xk, freqs_complex, device = x.device)
...
class EncoderBlock(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.attention_norm = RMSNorm(args.dim, eps=args.norm_ops)
        # Normalization BEFORE the feed forward block
        self.ffn_norm = RMSNorm(args.dim, ops=args.norm_ops)
...
class Transformer(nn.Module):
    ...
    def __init__(self, args: ModelArgs) -> None:
        ...
        self.norm = RMSNorm(args.dim, eps=args.norm_ops)
